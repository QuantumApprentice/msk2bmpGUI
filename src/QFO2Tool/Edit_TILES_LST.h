...
bool append_TMAP_tiles_LST(user_info* usr_nfo, tt_arr_handle* handle, proto_export* state/*char* save_buff*/);
// char* load_tiles_lst_game(char* game_path);
tile_name_arr* make_name_list_arr(char* new_tiles_list);

char* save_NEW_FRM_tiles_LST(tt_arr_handle* handle, char* save_path, proto_export* state);
char* load_LST_file(char* game_path, char* LST_path, char* LST_file);
char* append_FRM_tiles_LST(char* tiles_lst_path, tt_arr_handle* handle, proto_export* state);
bool load_FRM_tiles_LST(user_info* usr_nfo, proto_export* cur_state);
void set_false(proto_export* cur_state);
