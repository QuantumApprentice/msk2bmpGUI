...
void SDL_Color_Match(SDL_Surface* Convert,
    SDL_PixelFormat* pxlFMT_Pal, SDL_Surface* Temp_Surface);
void Euclidian_Distance_Color_Match(
    SDL_Surface* Convert, SDL_Surface* Temp_Surface);
void clamp_dither(SDL_Surface *Surface_32,
    union Pxl_err *err, int pixel_idx, int factor);
void limit_dither(SDL_Surface *Surface_32,
    union Pxl_err *err, int **pxl_index_arr);
