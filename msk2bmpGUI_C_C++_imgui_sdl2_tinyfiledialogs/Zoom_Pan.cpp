...
void viewport_boundary(image_data* img_data, ImVec2 size)
{
    ...
    if (size.x >= window_size.x) {
        ...
        img_data->offset.x = std::min((double)(window_size.x / 2         ), img_data->offset.x);
        // technically should update corner_pos here if the offset changed
    }
    else {
        img_data->offset.x = std::max((double)(-size.x / 2               ), img_data->offset.x);
        img_data->offset.x = std::min((double)(window_size.x - size.x / 2), img_data->offset.x);
        // technically should update corner_pos here if the offset changed
    }


    if (size.y >= window_size.y) {
        img_data->offset.y = std::max((double)(window_size.y / 2 - size.y), img_data->offset.y);
        img_data->offset.y = std::min((double)(window_size.y / 2         ), img_data->offset.y);
        // technically should update corner_pos here if the offset changed
    }
    else {
        img_data->offset.y = std::max((double)(-size.y / 2               ), img_data->offset.y);
        img_data->offset.y = std::min((double)(window_size.y - size.y / 2), img_data->offset.y);
        // technically should update corner_pos here if the offset changed
    }
